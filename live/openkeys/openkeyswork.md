...
`function authenticate($username, $password)` hints at passing variables username and passwords

are we checking if the user:password exists on the local machine? as a user?

 At this point I usually add the ip address to my `/etc/hosts` for easier navigation, adding `10.10.10.199     openkeys.htb`.\
...
exploit given at https://raw.githubusercontent.com/bcoles/local-exploits/master/CVE-2019-19520/openbsd-authroot

run exploit, get root flag